...
class ProductoBBDD extends Crud {
    ...
    public function getAllWithDetails() {
        $stmt = $this->conexion->prepare("
            SELECT 
                ...
                COALESCE((SELECT JSON_ARRAYAGG(url) FROM imagenes_productos WHERE id_producto_fk = productos.id_producto), '[]') AS imagenes
            FROM productos
        ...
        foreach ($result as &$row) {
            $row['imagenes'] = json_decode($row['imagenes']);
        }
    ...
    public function getByIdWithDetails($id) {
        ...
        if ($result) {
            $result['imagenes'] = json_decode($result['imagenes']);
        }
